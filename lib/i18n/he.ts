...
export const he: Dict = {
  ...
  simpleGuide: (level) => {
    ...
    if (level === "adv" || level === "champ" || level === "hero") {
      pages.push({
        i: "🧩",
        t: "פירוק לגורמים",
        x: 'ברמות הגבוהות יש בונוס מיוחד! 🧩<br><br>כשמגיעים למספר היעד, אפשר לפרק אותו לשני מספרים שכפל שלהם נותן אותו (למשל <span dir="ltr">24 = 4 × 6</span>) — ומקבלים גרגירים נוספים! 🦴<br><br>גם מעבר במשבצות עם המסגרת הזהובה (גורמים של היעד) שווה בונוס.',
      });
    }
    return pages;
  },
  ...
  factorPrompt: (n) =>
    `מצאו שני מספרים (כל אחד 2 ומעלה) שכפל שלהם נותן <strong>${n}</strong>.<br>פתרתם? תקבלו בונוס! 🦴`,
  factorCheck: "✅ בדקו",
  factorSkipBtn: "דלג",
  factorCorrect: (a, b, n, bonus) =>
    `🎉 כל הכבוד! <span dir="ltr">${a} × ${b} = ${n}</span> — בונוס +${bonus} 🦴`,
  factorWrong: (n) => `כמעט! צריך שני מספרים שכפל שלהם הוא ${n}. נסו שוב 😊`,
