  ...
  simpleGuide: (level) => {
    ...
    if (level === "adv" || level === "champ" || level === "hero") {
      pages.push({
        i: "🧩",
        t: "פירוק לגורמים",
        x: 'ברמות הגבוהות יש בונוס מיוחד! 🧩<br><br>כשמגיעים למספר היעד, אפשר לפרק אותו לשני מספרים שמכפלתם נותנת אותו (למשל <span dir="ltr">24 = 4 × 6</span>) — ומקבלים גרגירים נוספים! 🦴<br><br>גם מעבר במשבצות עם המסגרת הזהובה (גורמים של היעד) שווה בונוס.',
      });
    }
    return pages;
  },
